...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier le sous-chapitre</h1>

        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route('sous-chapitres.update', $sousChapitre) }}">
                @csrf
                @method('PUT')
                ...
                <!-- Contenu -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-1">Contenu</label>
                    <textarea name="contenu" rows="6"
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400">{{ old('contenu', $sousChapitre->contenu) }}</textarea>
                    @error('contenu')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
