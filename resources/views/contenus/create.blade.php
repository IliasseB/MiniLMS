...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Nouveau contenu</h1>
            ...
            <form method="POST" action="{{ route('contenus.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="texte" rows="6"
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400">{{ old('texte') }}</textarea>
                    @error('texte')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
